...
import type {MarketstackEodData, MarketstackApiResponse} from '@/types/marketstack';
...
export function TickerPriceSearch({
    ...
    // Clear analysis data when ticker changes, but preserve portfolio
    useEffect(() => {
        // Clear analysis-specific data to prevent stale data in subsequent analyses
        setProcessedChartData([]);
        setChartAnalysisResults([]);
        setOptionGreeks(new Map());
        setIsAnalyzing(false);

        // Restore portfolio from sessionStorage if available (don't clear it!)
        const cached = sessionStorage.getItem('portfolioData');
        if (cached) {
            try {
                const data: PortfolioParseResult = JSON.parse(cached);
                ...
                // Keep portfolio status as ready
                setUploadState(prev => ({
                    ...prev,
                    portfolio: { ...prev.portfolio, status: 'ready' },
                    charts: initialUploadState.charts,
                    research: initialUploadState.research
                }));
                ...
                console.log(`📦 [TICKER CHANGE] Restored portfolio from cache for: ${tickerSymbol}`);
            } catch (err) {
                console.error('❌ [TICKER CHANGE] Failed to restore portfolio:', err);
            }
        } else {
            // No cached portfolio - reset to initial state
            setUploadState(initialUploadState);
            setReadiness(initialReadiness);
            setParsedPortfolio(null);
        }

        console.log(`🔄 Cleared analysis data for ticker change to: ${tickerSymbol}`);
    }, [tickerSymbol]);
